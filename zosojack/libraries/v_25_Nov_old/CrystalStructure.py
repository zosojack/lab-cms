...
class CrystalStructure:
    ...
    def find_neighbours(self):
        ...
        NON considera il punto di giunzione polinomiale R_P.
        """
    ...
    def find_neighbours_matrix(self):
        ...
        - second_neighbour_matrix: matrice simmetrica di bool che indica se due atomi
                                   vanno congiunti con il polinomio di settimo ordine
        ...
        second_neighbour_matrix = np.zeros((n, n), dtype=bool)
        ...
        for i in range(n):
            for j in range(i+1, n):  # solo triangolo superiore
                ...
                # Se minore di R_P, aggiungo alla matrice dei vicini
                if d_ij <= self.R_P:
                    neighbour_matrix[i, j] = True
                    neighbour_matrix[j, i] = True  # simmetria
                    distance_matrix[i, j] = d_ij
                    distance_matrix[j, i] = d_ij
                # Se minore di R_C, ma maggiore di R_P, aggiungo alla matrice dei secondi vicini
                elif d_ij <= self.R_C:
                    second_neighbour_matrix[i, j] = True
                    second_neighbour_matrix[j, i] = True  # simmetria
                    distance_matrix[i, j] = d_ij
                    distance_matrix[j, i] = d_ij
        
        self.neighbour_matrix = neighbour_matrix
        self.second_neighbour_matrix = second_neighbour_matrix
        self.distance_matrix = distance_matrix 
        
    def find_neighbours_numpy(self):
        """
        Versione vettoriale di find_neighbours con NumPy, analoga a find_neighbours_matrix ma più efficiente.
        - neighbour_matrix: (<= R_P) prime vicine per Lennard-Jones
        - second_neighbour_matrix: (R_P < d <= R_C) coppie per giunzione polinomiale
        - distance_matrix: distanze per tutte le coppie con d <= R_C, altrimenti inf (diagonale a 0)
        """
        N = self.N_atoms
        pos = np.asarray(self.positions)              # (N,3)
        # matrice delle differenze e delle distanze (N,N)
        diffs = pos[:, None, :] - pos[None, :, :]     # (N,N,3)
        dist = np.linalg.norm(diffs, axis=2)          # (N,N)

        eye = np.eye(N, dtype=np.bool_)
        # maschere booleane simmetriche, esclusa la diagonale
        neighbour_matrix = (dist <= self.R_P) & (dist <= self.R_C) & (~eye)
        second_neighbour_matrix = (dist > self.R_P) & (dist <= self.R_C) & (~eye)

        # distance_matrix: distanze per tutte le coppie entro R_C, inf altrimenti
        any_mask = neighbour_matrix | second_neighbour_matrix
        distance_matrix = np.where(any_mask, dist, np.inf)
        # per comodità, imponiamo la diagonale a 0.0
